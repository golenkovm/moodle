...
Example:
\$sudo -u www-data /usr/bin/php admin/cli/restore.php --file=/path/to/backup/file.mbz --categoryid=1\n
EOL;
...
cli_heading(get_string('extractingbackupfileto', 'admin', $path));
$fp = get_file_packer('application/vnd.moodle.backup');
$fp->extract_to_pathname($options['file'], $path);

cli_heading(get_string('preprocessingbackupfile'));
try {
    $courseid = restore_dbops::create_new_course("_temp", "_temp", $options['categoryid']);
...
cli_heading(get_string('restoredcourseid', 'admin', $courseid));
exit(0);
