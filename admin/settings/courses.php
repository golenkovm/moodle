...
if ($hassiteconfig or has_any_capability($capabilities, $systemcontext)) {
    ...
    $temp->add(new autocomplete('backup/backup_auto_exclude_events',
        ...
        function() {
            ...
            foreach ($events as $eventclass => $eventdetails) {
                // Exclude read-only events as in anyways they are ignored
                // when a course is checked whether it's been modified or not.
                if ($eventdetails['crud'] != 'r') {
                    $choice = $eventdetails['eventname'] . ' (' . $eventdetails['eventclass'] . ')';
                    $choices[$eventclass] = $choice;
                }
            }
            return $choices;
        },
