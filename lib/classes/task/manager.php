...
class manager {
    ...
    public static function get_running_tasks($sort = ''): array {
        ...
        $sql = "SELECT subquery.*
                  FROM (SELECT concat('s', ts.id) as uid, ts.id, 'scheduled' as type, ts.component, ts.classname,
                               ts.nextruntime, (:now1 - ts.timestarted) as time, ts.blocking, ts.faildelay,
                               ts.customised, ts.disabled, '' as customdata, ts.timestarted, ts.hostname, ts.pid
                          FROM {task_scheduled} ts
                         WHERE ts.timestarted IS NOT NULL
                         UNION ALL
                        SELECT concat('a', ta.id) as uid, ta.id, 'adhoc' as type, ta.component, ta.classname,
                               ta.nextruntime, (:now2 - ta.timestarted) as time, ta.blocking, ta.faildelay,
                               0 as customised, 0 as disabled, ta.customdata, ta.timestarted, ta.hostname, ta.pid
                          FROM {task_adhoc} ta
